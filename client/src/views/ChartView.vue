<template>
  <div class="flex flex-col items-center h-[calc(100vh-36px)] overflow-y-auto">
    <div v-if="attackRecord != null && mtdRecord != null">
      ...
      <MTDOperation v-if="(store.parameters)['scheme'] != 'None'" :mtdRecord="mtdRecord" />
      <div v-if="compromiseMetrics != null && metricsItems > 0"
     id="compMetrics"
...
>
        ...
        <div id="metricsTitleBar" class="flex items-center justify-between">
          <div class="flex space-x-5 items-center">
            ...
            <div class="text-xl border border-[--text-color] pl-2 pr-2 rounded">Ratio: {{ compromiseMetrics[itemIndex].host_compromise_ratio * 100}}%</div>
          </div>
          <div class="flex space-x-2">
            <svg-icon v-if="itemIndex > 0" @click="showPreviousItem"
              type="mdi"
              size="20"
              class="rotate-90 border border-[--text-color] border-[1.5px] rounded-lg hover:bg-background-secondary hover:cursor-pointer"
              :path="mdiChevronDown"
            ></svg-icon>
            <svg-icon v-if="itemIndex < metricsItems - 1" @click="showNextItem"
              type="mdi"
              size="20"
              class="rotate--90 border border-[--text-color] border-[1.5px] rounded-lg hover:bg-background-secondary hover:cursor-pointer"
              :path="mdiChevronDown"
        ...
        <div id="metricsData" class="mt-5 overflow-x-hidden flex-grow">
          <div id="metricsContainer" class="flex space-x-3 transition-transform duration-700 ease-in-out">
            <div v-for="item in compromiseMetrics" :key="item.id" class="inline-block min-w-full">
              <div class="border border-[--text-color] p-2 rounded whitespace-nowrap">
                Attack Success Rate: {{ (item.attack_success_rate * 100).toFixed(2)}}% <br>
                MTD Execution Frequency: {{ (item.mtd_execution_frequency * 100).toFixed(2) }}% <br>
                Time to compromise: {{ item.time_to_compromise.toFixed(4) }}s <br>
              </div>
            </div>
          </div>
        </div>
