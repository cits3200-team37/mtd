...
const uniqueHosts = Array.from(new Set(Object.values(attackRecord.current_host_uuid)));
...
const attackRecordArray = Object.values(attackRecord.current_host_uuid)
  ...
  .map((uuid, i) => ({
    ...
    color: attackActionLegend.find((entry) => entry.name === attackRecord.name[i]).color,
  }));
...
const createChart = () => {
  ...
  const xScale = d3.scaleLinear().domain([0, d3.max(attackRecordArray, (d) => d.start_time + d.duration) * 1.05]).range([0, width]);
  const yScale = d3.scaleBand().domain(hostToken.map((d) => d.token)).range([0, height]).padding(0.1);
  ...
  svgContainer.append('g').attr('class', 'x-axis').attr('transform', `translate(0, ${height})`).call(xAxis);
  ...
  svgContainer
    ...
    .attr('y', (d) => yScale(d.currentHostToken) + 5)
    .attr('width', (d) => xScale(d.duration) * 1.1)
  ...
  const legend = svgContainer
    .append('g')
    .attr('class', 'legend')
    .attr('transform', `translate(10, ${height - (attackActionLegend.length * 15 + 20)})`);
