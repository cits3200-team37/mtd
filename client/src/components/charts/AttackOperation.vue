<template>
  ...
  const interruptLoc = Array.from(new Set(Object.values(attackRecord.interrupted_in)))
  ...
  const compromiseHostRecordArray = Object.values(attackRecord.compromise_host_uuid)
  ...
  const createChart = () => {
    ...
    const xScale = d3
      .scaleLinear()
      .domain([0,d3.max(attackRecordArray, (d) => d.start_time + d.duration) * 1.05,])
      .range([0, width]);
    const yScale = d3
      .scaleBand()
      .domain(uniqueNames)
