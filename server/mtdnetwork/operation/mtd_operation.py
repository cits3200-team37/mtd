...
class MTDOperation:
    def __init__(
        ...
        self._mtd_scheme = MTDScheme(
            network=network,
            scheme=scheme,
            mtd_trigger_interval=mtd_trigger_interval,
        ...
        while True:
            ...
            if len(resource.users) == 0:
                ...
                if mtd.get_priority() not in self.network.get_suspended_mtd():
                    ...
                    logging.info(
                        "MTD: %s suspended at %.1fs due to resource occupation"
                        % (mtd.get_name(), self.env.now + self._proceed_time)
                    )
            ...
            yield self.env.timeout(
                exponential_variates(
                    self._mtd_scheme.get_mtd_trigger_interval(),
                ...
                for priority in suspended_mtd_priorities:
                    ...
                    if len(resource.users) == 0:
                        ...
                        yield self.env.process(
                            self._mtd_execute_action(env=self.env, mtd=mtd)
                        )
            else:
                # register and trigger all MTDs
                if (
                    not self.network.get_suspended_mtd()
                    and not self.network.get_mtd_queue()
                ):
                    self._mtd_scheme.register_mtd()
    ...
    def _mtd_execute_action(self, env, mtd):
        ...
        logging.info(
            "MTD: %s deployed in the network at %.1fs." % (mtd.get_name(), start_time)
        )
        yield env.timeout(
            exponential_variates(
                mtd.get_execution_time_mean(), mtd.get_execution_time_std()
            )
        )
    ...
    def _interrupt_adversary(self, env, mtd):
        ...
        if attack_process is not None and attack_process.is_alive:
            if mtd.get_resource_type() == "network":
                ...
                logging.info(
                    "MTD: Interrupted %s at %.1fs!"
                    % (
                        self.attack_operation.get_adversary().get_curr_process(),
                        env.now + self._proceed_time,
                    )
                )
                self.network.get_mtd_stats().add_total_attack_interrupted()
            elif (
                mtd.get_resource_type() == "application"
                and self.attack_operation.get_adversary().get_curr_process()
                not in ["SCAN_HOST", "ENUM_HOST", "SCAN_NEIGHBOR"]
            ):
                self.attack_operation.set_interrupted_mtd(mtd)
                self.attack_operation.get_attack_process().interrupt()
                logging.info(
